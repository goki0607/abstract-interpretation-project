(** File printer.ml  © P. Cousot 2018 
  * Changes made by Goktug Saatcioglu 2019 
  * to obtain printerRedProdCongruenceInterval.ml *)
...
let string_of_redci p = (stringofaP p) ;;
let print_label l = print_string ((string_of_label l) ^ ": ");;
let print_ci l = print_string ((stringofaP l) ^ ": ");;
...
let print_labelling (at, atP, after, afterP, escape, break, breakP) m =
    let s = "<" ^ (string_of_label at) ^ ": " ^ (string_of_redci atP) ^ "; "
                ^ (string_of_label after) ^ ": " ^ (string_of_redci afterP) ^ "; "
                ^ (bool_to_string escape) ^ "; "
                ^ (string_of_label break) ^ ": " ^ (string_of_redci breakP) ^
            ">"  in
       print_string s;
...
let print_atP (at, atP, after, afterP, escape, break, breakP) = print_ci atP;;
let print_after (at, atP, after, afterP, escape, break, breakP) = print_label after;;
let print_afterP (at, atP, after, afterP, escape, break, breakP) = print_ci afterP;;
