(** File printer.ml  © P. Cousot 2018 
  ...
  * to obtain printerPotentialPointsTo.ml *)
...
let rec print_node s = match s with 
    ...
    | AssignP (p, a, l) -> print_string ("*" ^ p ^ " = "); print_aexpr a; print_string "; "
...
let string_of_location p = (stringofaP p) ;;
let print_label l = print_string ((string_of_label l) ^ ": ");;
let print_location l = print_string ((stringofaP l) ^ ": ");;
...
let print_labelling (at, atP, after, afterP, escape, break, breakP) m =
    let s = "<" ^ (string_of_label at) ^ ": " ^ (string_of_location atP) ^ "; "
                ^ (string_of_label after) ^ ": " ^ (string_of_location afterP) ^ "; "
                ^ (bool_to_string escape) ^ "; "
                ^ (string_of_label break) ^ ": " ^ (string_of_location breakP) ^
            ">"  in
       print_string s;
...
let print_atP (at, atP, after, afterP, escape, break, breakP) = print_location atP;;
let print_after (at, atP, after, afterP, escape, break, breakP) = print_label after;;
let print_afterP (at, atP, after, afterP, escape, break, breakP) = print_location afterP;;
let rec print_labelled_node s m = match s with 
    ...
    | AssignP (v, a, ls) -> print_labelling ls m; print_at ls; print_string ("*" ^ v ^ " = "); print_aexpr a; print_string "; "
