...
let ljoin l1 l2 =
  ...
  | NIL, _ -> l2
  | _, NIL -> l1
  | Int i1, Int i2 -> Int (I.ijoin i1 i2)
...
let lmeet l1 l2 =
  ...
  | NIL, _ -> NIL
  | _, NIL -> NIL
  | Int i1, Int i2 -> Int (I.imeet i1 i2)
...
let assignP x a r = function y ->
                      if (r x) = BOT then BOT else
                      if is_null (r x) then raise (ERR ("Error: Cannot dereference null pointer."))
                      else if is_int (r x) then raise (ERR ("Error: Cannot dereference integer value."))
                      else if (r x) = TOP then TOP
                      else if is_int (r y) then evala a r
                      else if (x=y) then
                        if (lleq (collect x r) (r x)) then ljoin (r x) (evala a r)
                        else if ((lleq (collect x r) (r x)) |> not) then ljoin (r x) BOT
                        else BOT
                      else (r y)

(* if StringSet.mem y (get_loc (r x)) then evala a r else *)    
...
let back_sum l1 l2 chi =
  match l1, l2, chi with
  | Int i1, Int i2, Int chi1 -> 
      let i1', i2' = I.back_iplus i1 i2 chi1 in
      Int (i1'), Int (i2')
  | _ -> raise (ERR ("Error: Can only do arithmetic with integers."))
...
let back_min l1 l2 chi =
  match l1, l2, chi with
  | Int i1, Int i2, Int chi1 ->
      let i1', i2' = I.back_imin i1 i2 chi1 in
      Int (i1'), Int (i2')
  | _ -> raise (ERR ("Error: Can only do arithmetic with integers."))
...
let rec back_evala a r chi =
  ...
  | Plus (a1, a2) ->
      let chi1, chi2 = back_sum (evala a1 r) (evala a2 r) chi in 
      meet (back_evala a1 r chi1) (back_evala a2 r chi2)
  | Minus (a1, a2) ->
      let chi1, chi2 = back_min (evala a1 r) (evala a2 r) chi in 
      meet (back_evala a1 r chi1) (back_evala a2 r chi2)

let back_lt l1 l2 =
  match l1, l2 with
  | Int i1, Int i2 -> 
      let i1', i2' = I.back_lt i1 i2 in
      Int (i1'), Int (i2')
  | _ -> raise (ERR ("Error: Can only do arithmetic comparison with integers."))

let back_not_lt l1 l2 =
  match l1, l2 with
  | Int i1, Int i2 ->
      let i1', i2' = I.back_not_lt i1 i2 in
      Int (i1'), Int (i2')
  | _ -> raise (ERR ("Error: Can only do arithmetic comparison with integers."))
...
let back_gt l1 l2 =
  match l1, l2 with
  | Int i1, Int i2 ->
      let i1', i2' = I.back_gt i1 i2 in
      Int (i1'), Int (i2')
  | _ -> raise (ERR ("Error: Can only do arithmetic comparison with integers."))

let back_not_gt l1 l2 =
  match l1, l2 with
  | Int i1, Int i2 ->
      let i1', i2' = I.back_not_gt i1 i2 in
      Int (i1'), Int (i2')
  | _ -> raise (ERR ("Error: Can only do arithmetic comparison with integers."))
